...
Lazy.use {
  ...
  config = function()
    ...
    --- classes
    --- FIXME do not assign "]c" and "[c" in diff mode
    vim.keymap.set({ "n", "x", "o" }, "]c", function() move.goto_next_start("@class.outer", "textobjects") end, { desc = "@class next" })
    vim.keymap.set({ "n", "x", "o" }, "[c", function() move.goto_previous_start("@class.outer", "textobjects") end, { desc = "@class prev" })
