...
local function setup()
  ...
  vim.keymap.set({ "v", "x" }, "<A-v>", mc.visualToCursors)

  -- Add or skip adding a new cursor by matching word/selection
  vim.keymap.set({ "v", "x" }, "<C-d>", function() mc.matchAddCursor(1) end)
  vim.keymap.set({ "n", "x" }, "<A-d>", function() mc.matchAddCursor(1) end)
  vim.keymap.set({ "n", "x" }, "<A-b>", function() mc.matchAddCursor(-1) end)
  vim.keymap.set({ "n", "x" }, "<A-s>", function() mc.matchSkipCursor(1) end)

  -- Add a cursor for all matches of cursor word/selection in the document
  vim.keymap.set({ "n" }, "<A-m>", mc.matchAllAddCursors)
  vim.keymap.set({ "v", "x" }, "<A-m>", matchAllAddCursorsVisual)
  ...
  mc.addKeymapLayer(function(layerSet)
    ...
    layerSet({ "n", "x" }, "<A-j>", mc.nextCursor)
    layerSet({ "n", "x" }, "<A-l>", mc.nextCursor)

    layerSet({ "n", "x" }, "<A-k>", mc.prevCursor)
    ...
    layerSet("n", "<esc>", function()
      if mc.cursorsEnabled() then mc.clearCursors() else mc.enableCursors() end
    end)
  end)
end
