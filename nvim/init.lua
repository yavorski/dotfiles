      ...
      providers = {
        lsp = {
          ...
          transform_items = function (ctx, items)
            ...
            -- remove always suggested closing tag by emmet-ls
            if vim.tbl_contains({ "html", "razor", "cshtml" }, vim.bo.filetype) then
              items = vim.tbl_filter(function(item) return not vim.startswith(item.label, "</") end, items)
            end
