...
-- fuzzy matching for built-in completion
...
local function buffer_only(discard_modified_buffers)
  ...
  if current_buffer_ft == "NvimTree" or current_buffer_ft == "dbui" or current_buffer_ft == "dbout" then
    return vim.api.nvim_notify(current_buffer_ft, vim.log.levels.INFO, {})
  end
...
vim.keymap.set({ "n", "v", "x" }, "<leader>x", [["+d]], { silent = true, desc = "[sc] Yank Delete" })
...
function Dark.lualine()
  ...
  line.command = colorize(colors.softpink)
...
LSP.buffer_keymaps = function(buffer)
  ...
  -- <CTRL-W-d> is default
  keymap("n", "<leader>E", vim.diagnostic.open_float, "LSP Diagnostic Open Float")
...
--------------------------------------------------------------
-- LSP enable semantic tokens for whole buffer
-- https://github.com/seblyng/roslyn.nvim/wiki#semantic-tokens
--------------------------------------------------------------
LSP.enable_full_range_semantic_tokens = function(client)
  if client.is_frst_enabled then return end
  client.is_frst_enabled = true
  ...
  client.request = function(method, params, handler, req_bufnr)
    ...
    return request_inner("textDocument/semanticTokens/range", {
      ...
      range = {
        ...
        ["end"] = { line = line_count - 1, character = string.len(last_line) - 1 }
      }
    }, handler, req_bufnr)
  end
end
  ...
  opts = {
    config = {
      on_attach = function(client)
        LSP.enable_full_range_semantic_tokens(client)
      end
    }
  }
}
  ...
  opts = {
    ...
    sources = {
      ...
      providers = {
        lsp = {
          name = "LSP",
          fallbacks = {}, -- enable buffer completions
          score_offset = 1024,
          should_show_items = function(ctx, items)
            -- remove always suggested closing tag by html server
            if #items == 1 and vim.tbl_contains({ "html", "razor", "cshtml" }, vim.bo.filetype) and vim.startswith(items[1].label, "</") then
              return false
            end
            return true
          end
        },
        path = { name = "Path", max_items = 10, score_offset = 512 },
        buffer = { name = "Buffer", max_items = 10, score_offset = 256 },
        snippets = { name = "Snippets", score_offset = 128 },
        dadbod = {
          name = "Dadbod",
